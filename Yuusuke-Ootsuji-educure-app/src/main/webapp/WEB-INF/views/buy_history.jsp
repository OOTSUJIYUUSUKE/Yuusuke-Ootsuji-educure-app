...
<body>
    <div class="container">
        ...
        <main>
            ...
            <div class="purchase-history-message">
			    <c:choose>
				    <c:when test="${fn:length(products) == 0}">
                        購入履歴は見つかりませんでした。
                    </c:when>
				    <c:otherwise>
                        ${fn:length(products)}件の購入履歴が見つかりました。
                    </c:otherwise>
			</c:choose>
			</div>
			<div class="product-list" id="product-list">
				<c:forEach items="${products}" var="product">
                    <a href="${pageContext.request.contextPath}/buy_product_detail?product_id=${product.productId}" class="product-link">
